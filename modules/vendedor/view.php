...
class VendedorView extends View {
	function panel() {
		$gui = file_get_contents("static/modules/vendedor/panel.html");
		$render = $this->render_breadcrumb($gui);
		$template = $this->render_template($render);
		print $template;
	}

	function listar($vendedor_collection, $frecuenciaventa_collection, $vendedorobj_collection) {
		$user_level = $_SESSION["data-login-" . APP_ABREV]["usuario-nivel"];
		switch ($user_level) {
			case 2:
				$gui = file_get_contents("static/modules/vendedor/listar_supervisor.html");
				$tbl_vendedor_array = file_get_contents("static/modules/vendedor/tbl_vendedor_array_supervisor.html");
				break;
			default:
				$gui = file_get_contents("static/modules/vendedor/listar.html");
				$tbl_vendedor_array = file_get_contents("static/modules/vendedor/tbl_vendedor_array.html");
				break;
		}
		...
		foreach ($vendedorobj_collection as $vendedor) unset($vendedor->infocontacto_collection);
		...
		$gui_slt_vendedor = $this->render_regex('SLT_VENDEDOR', $gui_slt_vendedor, $vendedorobj_collection);
		$render = str_replace('{slt_frecuenciaventa}', $gui_slt_frecuenciaventa, $gui);
	...
	function agregar($provincia_collection, $documentotipo_collection, $frecuenciaventa_collection) {
		$gui = file_get_contents("static/modules/vendedor/agregar.html");
	...
	function editar($provincia_collection, $documentotipo_collection, $frecuenciaventa_collection, $obj_vendedor) {
		$gui = file_get_contents("static/modules/vendedor/editar.html");
		$gui_slt_provincia = file_get_contents("static/common/slt_provincia.html");
		$gui_slt_documentotipo = file_get_contents("static/common/slt_documentotipo.html");
		$gui_slt_frecuenciaventa = file_get_contents("static/common/slt_frecuenciaventa.html");
		$gui_lst_input_infocontacto = file_get_contents("static/modules/vendedor/lst_input_infocontacto.html");
	...
	function consultar($egresocomision_collection, $sum_importe_producto, $sum_cantidad_producto, $obj_vendedor, $estadisticas) {
		$gui = file_get_contents("static/modules/vendedor/consultar.html");
		$gui_tbl_pago_egresocomision = file_get_contents("static/modules/vendedor/tbl_pago_egresocomision.html");
		$gui_tbl_sum_importeventa_producto = file_get_contents("static/modules/vendedor/tbl_sum_importeventa_producto.html");
		$gui_tbl_sum_cantidadventa_producto = file_get_contents("static/modules/vendedor/tbl_sum_cantidadventa_producto.html");
		$gui_lst_infocontacto = file_get_contents("static/common/lst_infocontacto.html");
		if ($obj_vendedor->documentotipo->denominacion == 'CUIL' OR $obj_vendedor->documentotipo->denominacion == 'CUIT') {
			$cuil1 = substr($obj_vendedor->documento, 0, 2);
			$cuil2 = substr($obj_vendedor->documento, 2, 8);
			$cuil3 = substr($obj_vendedor->documento, 10);
			$obj_vendedor->documento = "{$cuil1}-{$cuil2}-{$cuil3}";
		}
